...
ee_u16 core_bench_list(core_results *res, ee_s16 finder_idx) {
...
#if LIST_DEBUG
		ee_printf("retval=%d,found=%d,missed=%d\n",retval,found,missed);
#endif
	...
	while (finder) {
#if LIST_DEBUG
		ee_printf("data16=%d,retval=%d\n",list->info->data16,retval);
#endif
		retval=crc16(list->info->data16,retval);
		finder=finder->next;
	}
#if LIST_DEBUG
	ee_printf("List sort 1: %d\n",retval);
#endif
...
list_head *core_list_init(ee_u32 blksize, list_head *memblock, ee_s16 seed) {
	...
	finder=list;
	while (finder) {
		ee_printf("[%d,%d]",finder->info->idx,(ee_u16)finder->info->data16);
		finder=finder->next;
	}
	ee_printf("\n");
#endif
